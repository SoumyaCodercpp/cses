...
long long query(int start, int end,int index,int l,int r) {
    if(l<=start&&end<=r) return segTree[index];
 
    if(l>end||r<start) return INT_MAX;
 
    int mid = (start+end)/2;
    ll ansl=query(start,mid,2*index+1,l,r);
    ll ansr=query(mid+1,end,2*index+2,l,r);
...
int main(){
    ...
    f(0,q,1){
        ...
        cin>>a1>>b;
            ...
            cout<<query(0,n-1,0,a1-1,b-1)<<endl;
